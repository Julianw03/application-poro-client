...
public class TFTDamageSkinsInventoryManager extends ArrayDataManager {

    private static final Pattern uriPattern = Pattern.compile("/lol-inventory/v2/inventory/TFT_DAMAGE_SKIN$");

    public TFTDamageSkinsInventoryManager(Starter starter) {
        super(starter);
    }

    @Override
    protected Optional<JsonArray> fetchCurrentState() {
        HttpsURLConnection con = starter.getConnectionManager().buildConnection(ConnectionManager.conOptions.GET, "/lol-inventory/v2/inventory/TFT_DAMAGE_SKIN");
    ...
    public String getEventName() {
        return DataManager.UPDATE_TYPE.ARRAY_OWNED_TFT_DAMAGE_SKINS.name();
    }
